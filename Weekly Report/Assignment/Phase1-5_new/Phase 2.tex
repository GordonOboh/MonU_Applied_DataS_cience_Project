...
\noindent
The project required a dataset of US rental housing listings containing: the listing price as the target variable, physical attributes (square footage, bedrooms, bathrooms), amenity flags (pets, smoking, wheelchair access, EV charging, furnished), property type, and geographic identifiers (state, region, latitude, longitude). The data needed to include Southern US states and apartments specifically (the project scope).

The target of approximately 100,000+ records was determined by considering the requirements of tree-based regression models. With roughly 15--20 candidate features (physical attributes, amenity flags, geographic identifiers), a rule-of-thumb of 5,000--10,000 records per feature for stable ensemble training suggested a minimum of 75,000--200,000 records. The 100K target sits comfortably within this range and ensures sufficient data for an 80/20 train-test split without starving the training partition. The selected Kaggle dataset (384,977 raw rows) exceeds this requirement even after filtering to the Southern-apartment scope, providing ample records for the subset.
...
\noindent
No difficulties were encountered during data gathering, access, or loading. Initial inspection revealed the presence of extreme outliers (e.g., maximum price over \$2.77B, maximum square footage over 8.39M), \$0 listings, and missing values in several columns; these issues are documented in Task 2.4 (Verifying Data Quality) and will be addressed during Phase~3 (Data Preparation).
...
\begin{table}
    ...
    \hline
        ...
        \hline
        \hline
        Numeric & price, sqfeet, beds, baths & Target: price. Shows extreme outliers (max \$2.77B, 8.39M sqft). Typical median values: \$1,036, 949 sqft, 2 beds, 1 bath. \\
        \hline
...
\noindent
Pie charts for the six binary amenity flags and two multi-class categoricals are shown in Figure~\ref{fig:pie}. The proportions below are derived from \texttt{df.describe()} applied to the binary columns of the full 384,977-row dataset: pandas computes the mean of a 0/1 binary column as the fraction of rows equal to 1, directly giving the percentage of listings where the amenity is present. Pet and smoking policies skew permissive: 72.7\% of listings allow cats, 70.8\% allow dogs, and 73.2\% allow smoking. Wheelchair access (8.2\%), EV charging (1.3\%), and furnished units (4.8\%) are minority classes. Laundry and parking options show meaningful class imbalance: the most common laundry arrangement (``w/d in unit'') and parking type (``off-street parking'') each account for a plurality of listings, with ``Not Specified'' representing a substantial share in both.
...
\noindent
Pearson and Spearman correlation heatmaps (Figure~\ref{fig:heatmap}) were computed using \texttt{df.corr(method='pearson')} and \texttt{df.corr(method='spearman')} in pandas, comparing every numeric and target-encoded feature against \texttt{price}. The values reported are the correlation coefficients extracted from the resulting matrices for each feature pair. Region shows the strongest correlation with price (Pearson $r \approx 0.73$, Spearman $\rho \approx 0.72$), confirming that geographic location is the dominant price driver. State follows (Pearson $r \approx 0.53$, Spearman $\rho \approx 0.49$). Latitude shows negligible linear correlation and Spearman rank correlation, suggesting a non-linear geographic component to pricing.
...
\begin{itemize}[itemsep = -9pt]
    \item Geographic location (region, state) will be the strongest predictors of rental price, consistent with the observed Pearson ($r \approx 0.73$ for region) and Spearman ($\rho \approx 0.72$ for region) correlations.
...
\begin{itemize}[itemsep = -9pt]
    ...
    \item Negative latitude: the minimum latitude of $-43.53^\circ$ falls in the southern hemisphere and is physically impossible for any US location, indicating a probable sign error in the source data.
\end{itemize}
...
\begin{table}[H]
    ...
    \hline
        ...
        \hline
        \hline
        parking\_options	& 140,687	& 36.54\% & Major & Create ``Missing'' category to preserve all records \\
        \hline
        laundry\_options	& 79,026	& 20.53\% & Major & Create ``Missing'' category to preserve all records \\
        \hline
...
\noindent
The dataset is of sufficient quality to proceed to Phase~3 (Data Preparation), provided the remedies outlined above are applied. The two most impactful issues are (1) the 36.54\% missing rate in \texttt{parking\_options}, which can be preserved via a ``Missing'' level rather than through deletion, and (2) extreme price/square-footage outliers that would distort model training if left untreated. No data collection rework is required.
