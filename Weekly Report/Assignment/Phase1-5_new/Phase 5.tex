...
%\vspace{-12pt}
\noindent
The metrics in Table \ref{tab:model_performance} serve distinct purposes in evaluating rental price predictions. MAE (\$35.58--\$47.07) reports the average absolute dollar error and is the most interpretable metric for a business audience: Random Forest's MAE of \$35.58 means that, on average, its predicted rent differs from the actual rent by about \$36. MSE and SSE reflect squared errors and penalise large mistakes more heavily; the large SSE values (e.g., 170M for Random Forest) are expected given 28,421 test records and do not indicate poor fit -- they are dataset-size-dependent. RMSE (\$77.32--\$85.70) brings the squared error back to dollar units and is directly comparable across models. R\textsuperscript{2} (0.934--0.946) measures the proportion of variance in price explained by the model and is scale-independent: a value of 0.946 means the model accounts for 94.6\% of the price variation in the test set, leaving only 5.4\% unexplained. This is strong for housing data, where factors like lease terms, property condition, and negotiation outcomes are not captured.
...
\noindent
Location clustering via K-Means (n\_clusters=148) substantially improved model performance across all three algorithms. The location\_cluster feature emerged as the number-one predictor in every model, confirming that this engineering decision was the single most impactful of the project.

\noindent
Target encoding was an effective choice for high-cardinality categorical features (region: 148 levels, location\_cluster: 148 levels). It compressed each category to a single numeric value without the dimensionality explosion of one-hot encoding, reducing per-model training time from an estimated 60--90 minutes to under one minute while preserving predictive signal.

\subsubsection{What Did Not Work}
\noindent
Initial attempts at K-Means with fewer clusters (50, then 125) produced weaker results until the cluster count was matched to the number of Craigslist regions (148). This suggests that the default elbow-method or heuristic approaches to choosing k would have been less effective than domain-informed selection.
