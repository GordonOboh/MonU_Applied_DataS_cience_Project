\section{Deployment}

\subsection{Plan Deployment}
\label{subsec:Plan Deployment}

\noindent
Although this academic project does not involve deploying a live production system, this section outlines a deployment plan that would apply if the Random Forest model were to be used in a real-world setting.

\subsubsection{Model Packaging and Serialization}

\noindent
The trained Random Forest model would be serialised using \texttt{joblib}, preserving the fitted pipeline including the TargetEncoder transformations and the K-Means location cluster mapping. This serialised model file would serve as the deployable artefact, versioned alongside the project code in a git repository.

\subsubsection{Deployment Architecture}

\noindent
A lightweight REST API would be the most practical deployment approach for this project. A Flask or FastAPI application would expose a single prediction endpoint accepting listing features as JSON input and returning a predicted monthly rent. The API would load the serialised model at startup and perform all necessary preprocessing (imputation of missing amenity fields, encoding of categorical variables, and location cluster assignment) before generating a prediction.

\noindent
The API would be containerised using Docker and hosted on a cloud platform such as AWS Elastic Beanstalk, Heroku, or Render. This approach keeps infrastructure costs near zero for low traffic while remaining scalable if usage grows. A simple web form frontend could be added as an optional layer, allowing non-technical users to enter property details (square footage, number of bedrooms, amenities, location) and receive an instant price estimate.

\subsubsection{Documentation Requirements}

\noindent
Three pieces of documentation would be produced for deployment:

\begin{itemize}
    \item \textbf{Technical Documentation:} Instructions for loading the model, running the API locally, and deploying to a cloud platform. This would include environment setup, dependency management (Python packages listed in \texttt{requirements.txt}), and configuration options.
    \item \textbf{User Guide:} A brief guide for stakeholders explaining what the model predicts, what data it requires, how to interpret the output, and what its limitations are: specifically that predictions are less reliable for high-price listings.
    \item \textbf{Data Dictionary:} A reference listing all input features, their types, allowed values, and whether they are required or optional, so that users can prepare data correctly.
\end{itemize}

\subsubsection{Specific Implementation Tasks}
\label{subsec:Implementation Tasks}

\noindent
The following concrete tasks enumerate what is required to move from the current research prototype to a deployable system. Each task includes the specific action, the tools involved, and the expected effort.

\medskip\noindent
\textbf{Task 1: Refactor preprocessing into a production pipeline.}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Action:} Convert the Jupyter Notebook preprocessing steps into a standalone Python module (\texttt{preprocess.py}) with functions for each stage: filtering, IQR outlier removal by region, K-Means clustering, median imputation, and target encoding.
    \item \textbf{Tools:} Python 3.12, scikit-learn 1.8.0, pandas 3.0.1, category-encoders 2.9.0, joblib 1.5.3.
    \item \textbf{Effort:} 1--2 days.
    \item \textbf{Validation:} Run the module end-to-end on a copy of the raw CSV and confirm that the output matrix dimensions match the current research pipeline (113,682 training rows $\times$ 16 features).
\end{itemize}

\medskip\noindent
\textbf{Task 2: Serialize the trained Random Forest model.}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Action:} Fit the final Random Forest on the full cleaned dataset (all 142,103 Southern apartment records), then serialize using \texttt{joblib.dump()}. Include the fitted TargetEncoder and K-Means objects in the same artifact.
    \item \textbf{Tools:} joblib, scikit-learn.
    \item \textbf{Effort:} 1 day, including training time.
    \item \textbf{Output:} \texttt{rf\_model\_v1\_2026-07-26.joblib} (estimated 200--400 MB).
\end{itemize}

\medskip\noindent
\textbf{Task 3: Build and containerize a REST API.}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Action:} Create a FastAPI application with a single \texttt{POST /predict} endpoint accepting a JSON body of listing features. The endpoint loads the serialized model at startup, applies all preprocessing steps, and returns \texttt{\{"predicted\_rent": 1025.00\}}.
    \item \textbf{Tools:} FastAPI 0.115+, Uvicorn, Docker, Docker Compose.
    \item \textbf{Effort:} 2--3 days.
    \item \textbf{Validation:} Send 50 test listings with known prices; confirm that the test-set MAE does not degrade significantly from the research-phase baseline (\$35.58).
\end{itemize}

\medskip\noindent
\textbf{Task 4: Set up cloud hosting.}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Action:} Push the Docker image to a container registry (Docker Hub or AWS ECR) and deploy to AWS Elastic Beanstalk or similar. Configure a custom domain and HTTPS certificate.
    \item \textbf{Tools:} AWS CLI, Docker, Terraform (optional).
    \item \textbf{Effort:} 1--2 days, plus DNS propagation time.
    \item \textbf{Validation:} Hit the production endpoint from an external client and verify a 200 response with a valid prediction.
\end{itemize}

\medskip\noindent
\textbf{Task 5: Build the monitoring dashboard.}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Action:} Create a weekly-report generation script that loads the latest batch of predictions, compares them to ground-truth prices, and outputs a PDF with MAE, RMSE, R\textsuperscript{2}, and drift-test results.
    \item \textbf{Tools:} Python, matplotlib, Jinja2 (for HTML reports), cron (scheduling).
    \item \textbf{Effort:} 2--3 days.
    \item \textbf{Validation:} Run the script on the research test set and confirm the report matches the known metrics.
\end{itemize}

\medskip\noindent
\textbf{Task 6: Write documentation.}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Action:} Produce three documents: \texttt{README.md} (technical setup), \texttt{USER\_GUIDE.md} (stakeholder instructions), and \texttt{DATA\_DICTIONARY.md} (feature reference).
    \item \textbf{Effort:} 1 day.
    \item \textbf{Validation:} A new team member should be able to run the API locally using only the README.
\end{itemize}

\subsubsection{Training and Support}

\noindent
A short orientation session would be provided to any stakeholders who would use the system directly. This would cover how to submit a prediction request, how to read the results, and what to do if the output seems unreasonable. Ongoing support would be handled through a single point of contact (the project lead), with a documented escalation path for data quality or model performance issues.

\subsection{Plan Monitoring and Maintenance}
\label{subsec:Plan Monitoring and Maintenance}

\noindent
A monitoring and maintenance plan is essential to ensure the model remains accurate and useful over time, since housing market conditions, listing practices, and rental prices all change.

\subsubsection{Performance Monitoring}

\noindent
Once deployed, the model's prediction accuracy should be tracked continuously. The key metrics to monitor are the same ones used during evaluation:

\begin{itemize}
    \item \textbf{Mean Absolute Error (MAE):} The average dollar deviation between predicted and actual rent. A significant increase from the baseline of \$35.58 would signal that the model has degraded.
    \item \textbf{R\textsuperscript{2}:} The proportion of variance explained. A drop from the baseline of 0.946 would indicate that the model is losing its predictive power.
    \item \textbf{Prediction drift:} The average prediction across all new requests should remain reasonably close to the training-period average of approximately \$1,016. A sustained shift could indicate that the market has changed.
\end{itemize}

\noindent
These metrics would be calculated on a rolling basis as actual rental prices become available. A dashboard (or a simple weekly report) would flag any metric that exceeds a predefined threshold: for example, a MAE increase of more than 20\% above the baseline.

\subsubsection{Data Drift Detection}

\noindent
Even before ground-truth prices are available, the model's input data should be monitored for drift. If the distribution of features such as square footage, number of bedrooms, or location shifts significantly from the training set, the model's predictions may become unreliable even if the model itself has not changed. Automated scripts would compare incoming data distributions to the training set on a monthly basis using statistical tests (e.g., Kolmogorov-Smirnov for numeric features, chi-square for categorical features). Any significant drift would trigger an alert and prompt a review of whether retraining is needed.

\subsubsection{Retraining Schedule}

\noindent
The model should be retrained on a regular schedule to remain current with market conditions. A quarterly retraining cycle is recommended, with the following triggers for off-cycle retraining:

\begin{itemize}
    \item A sustained MAE increase of more than 20\% above baseline.
    \item A statistically significant data drift alert persisting for two consecutive months.
    \item A major market event (e.g., a significant shift in regional housing policy or economic conditions).
\end{itemize}

\noindent
Retraining would follow the same pipeline used in the original project: data collection, cleaning with region-based IQR filtering, feature engineering including K-Means clustering, and model training with hyperparameter tuning. The retrained model would be evaluated against the current model on a held-out test set before replacement.

\subsubsection{Version Control and Governance}

\noindent
Every model version would be tracked. Each serialised model file would be named with a version number and date (e.g., \texttt{rf\_model\_v2\_2026-10-01.joblib}) and stored alongside a changelog documenting what changed, why, and the evaluation metrics for that version. The source code, training scripts, and evaluation results would remain in the project git repository, ensuring full reproducibility of every deployed model.

\subsubsection{Maintenance Responsibilities}

\noindent
For a small-scale academic project, a single data scientist or analyst would be sufficient to handle monitoring and maintenance. The responsibilities would include:

\begin{itemize}
    \item Reviewing the weekly performance dashboard and investigating any alerts.
    \item Running the monthly drift detection scripts and documenting results.
    \item Executing the quarterly retraining cycle and validating the new model.
    \item Updating the documentation when the model or pipeline changes.
\end{itemize}

\noindent
If the project were to expand to a larger deployment, these responsibilities would be distributed across a team with dedicated roles for data engineering, model validation, and stakeholder communication.

\subsection{Produce Final Report}
\label{subsec:Produce Final Report}

\noindent
The final report consolidates all six CRISP-DM phases into a single document, presenting the complete project narrative from business understanding through deployment planning. The report includes all deliverables produced during the project: project scope, data description, data preparation rationale, model descriptions and parameter settings, evaluation results, and the deployment plan outlined in this phase. The report concludes with a summary of lessons learned and recommendations for future work, as documented in Phase~5 (Reviewing the Process and Determining Next Steps).

\subsection{Review Project}
\label{subsec:Review Project}

\noindent
A retrospective review of the project was conducted to assess the overall experience and identify areas for improvement in future data science projects.

\subsubsection{What Went Well}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item The CRISP-DM framework provided a clear, structured path through a complex analytical task, preventing scope creep and ensuring no major step was skipped.
    \item The Kaggle Craigslist dataset, despite its quality issues, contained sufficient breadth (all 50 states, 12 property types) and depth (384,977 records, 18 features) to support the research objectives after filtering.
    \item Tree-based ensemble models proved well-suited to the mixed data types, handling both numeric features and target-encoded categoricals effectively without manual normalization.
    \item Region-sensitive preprocessing (per-region IQR filtering, location clustering) produced consistently better results than global approaches, confirming that local market structure matters.
\end{itemize}

\subsubsection{What Could Be Improved}
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Earlier documentation:} Several decisions made during data preparation and modeling were not documented in real time, requiring retrospective reconstruction. Future projects should document decisions as they are made.
    \item \textbf{Scope management:} The initial scope was broad (ranking amenity importance), but the results showed that amenities are secondary to location and size. Narrowing the research questions earlier could have focused effort on higher-impact analysis.
    \item \textbf{Time allocation:} The modeling phase consumed disproportionate time relative to business understanding and deployment planning. A more balanced distribution across CRISP-DM phases would produce a more complete project.
    \item \textbf{Tooling consistency:} Switching between Jupyter Notebook and standalone Python scripts created versioning challenges. Using a single environment with source control from the start would improve reproducibility.
\end{itemize}

\subsubsection{Team Experience}
\noindent
As a solo data scientist on this project, every phase was executed from research, coding, and documentation. While the breadth of work was challenging, the end-to-end experience of moving from a raw Kaggle CSV through to a deployable model provided a comprehensive understanding of the CRISP-DM lifecycle that would be difficult to gain by working on a single phase of a team project.
