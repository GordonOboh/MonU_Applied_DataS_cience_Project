...
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textbf{Personnel:} Single data scientist (Gordon Oboh) responsible for all phases of the project, with an academic advisor (Professor Nicholas Nardi) providing feedback on deliverables.
    \item \textbf{Computing resources:} 2 vCPU, 2 GB RAM, Ubuntu 24.04 LTS Server. No GPU available.
    \item \textbf{Data availability:} The Kaggle dataset is fully downloaded and accessible in CSV format (37.2 MB working file). No APIs, streaming sources, or database connections are required.
    \item \textbf{Assumptions:} (1) Craigslist listings in January 2020 are representative of the broader Southern apartment rental market; (2) listing price approximates actual transaction price; (3) amenity flags reported by submitters are truthful; (4) the CRISP-DM framework will be followed throughout the project.
    \item \textbf{Terminology:} \textit{Amenity} — a desirable feature of a rental property (pet policy, laundry, parking, etc.). \textit{Listing price} — the monthly rent advertised on Craigslist. \textit{Southern USA} — the 17 states per the US Census Bureau definition (AL, AR, DC, DE, FL, GA, KY, LA, MD, MS, NC, OK, SC, TN, TX, VA, WV).
\end{itemize}
...
\vspace{-9pt}
\begin{itemize}[itemsep=-9pt]
    \item \textit{Incomplete amenity data:} Craigslist relies on user-submitted listings with optional fields. Submitters may skip detailed amenity information (parking, laundry, pet policies) if they are in a hurry or consider it secondary to the listing text. If too many listings omit these fields, there may not be enough data to assess which amenities drive price. \textbf{Why this is a risk:} amenity features are the core independent variables for the analytical goal of ranking amenity importance; if they are missing from most records, the project's primary research question cannot be answered. \textbf{Contingency:} treat missing amenity fields as a separate category rather than discarding those listings.
    \item \textit{Erroneous or test listings:} Public classifieds platforms have no validation on price or size entries. Users may enter placeholder values (e.g., \$0, 1 sq ft) instead of accurate figures, or post test listings that are never removed. \textbf{Why this is a risk:} these records would introduce noise into the training data, misleading the regression model and producing unreliable price predictions and feature importance rankings. \textbf{Contingency:} apply threshold-based filters to remove entries with impossible values before training.
    \item \textit{Extreme price variation across markets:} Rental prices differ dramatically between a rural Mississippi town and downtown Atlanta. When analyzed together, legitimate high-end listings may appear as statistical outliers, and a single global outlier treatment could discard valid data from expensive markets. \textbf{Why this is a risk:} losing the high-end segment of the market would reduce the model's coverage and bias predictions toward mid-range listings, making the tool less useful for landlords and investors operating in premium segments. \textbf{Contingency:} apply outlier removal at the regional or state level so that each market's price distribution is treated independently.
\end{itemize}
...
Hardware       & 2 vCPU, 2 GB RAM, Ubuntu 24.04 LTS Server \\
\hline
\end{tabular}
\end{center}}%
